#include <ppt_3d/min_distance_to_hull_calculator.hpp>
...
void MinDistanceToHullCalculator::setConvexHullPointsAndEdges(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& convex_hull_cloud)
{   
    hull_points_.clear();
    normalized_hull_edge_vecs_.clear();
    const pcl::PointXYZ* pcl_pt;
...
float MinDistanceToHullCalculator::computeMinDistanceToHull(const pcl::PointXYZ& object_point)
{
...
float MinDistanceToHullCalculator::computeMinDistanceToHull(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& object_cloud)
{
